    ...
    return (
      <div className="bg-popover border text-popover-foreground rounded-lg shadow-md p-3 min-w-[200px]">
        <div className="font-semibold mb-2 border-b pb-1 flex justify-between items-center">
          <span>{label}</span>
          {isFuture && (
            <span className="text-[10px] bg-muted px-2 py-0.5 rounded text-muted-foreground uppercase tracking-wider">
              Previsão
            </span>
          )}
        </div>
...
export function ExpensesForecastChart({ data }: { data: Record<string, unknown>[] }) {
