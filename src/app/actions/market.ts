...
import { db } from "@/db";
import { marketItems, marketReceipts, marketReceiptTransactions, settings, transactions } from "@/db/schema";
...
export type MarketReceiptInput = { type: "image"; base64: string; mimeType: string } | { type: "text"; text: string };
  ...
  const recent = await db
    ...
    .where(
      and(
        ...
        lte(transactions.dueDate, futureLimit),
      ),
    )
    .orderBy(desc(transactions.dueDate), desc(transactions.id))
    .limit(50);

  return recent;
}

export async function saveMarketReceipt(receiptData: MarketReceiptData, selectedTransactionIds: number[]) {
  try {
    const session = await auth();
    if (!session?.user?.id) throw new Error("Unauthorized");
    const userId = session.user.id;
...
export async function getMarketReceipts(competencyMonth: string) {
  ...
  return await db
    ...
    .from(marketReceipts)
    .where(
      and(eq(marketReceipts.userId, userId), gte(marketReceipts.date, startDate), lte(marketReceipts.date, endDate)),
    )
...
export async function getMarketReceiptDetails(receiptId: string) {
  ...
  return await db.select().from(marketItems).where(eq(marketItems.receiptId, receiptId));
}
...
export async function getMarketDashboardData(competencyMonth: string) {
  ...
  const items = await db.select().from(marketItems).where(inArray(marketItems.receiptId, receiptIds));
  ...
  const itemsByCategory = Array.from(categoryCountMap.entries())
    .map(([category, count]) => ({
      category,
      count,
    }))
  ...
  const itemMap = new Map<
    string,
    { description: string; netPrice: number; quantity: number; unitMeasure: string; category: string }
  >();
  items.forEach((item) => {
  ...
  const allItems = Array.from(itemMap.values()).sort((a, b) => b.netPrice - a.netPrice);

  const topExpensiveItems = allItems.slice(0, 5);

  const categoryItemsMap = new Map<
    string,
    { description: string; netPrice: number; quantity: number; unitMeasure: string }[]
  >();
  items.forEach((item) => {
    const cat = item.category || "Outros";
  ...
  const topItemsByCategory = Array.from(categoryItemsMap.entries())
    ...
    .sort(
      (a, b) =>
        b.items.reduce((acc, curr) => acc + curr.netPrice, 0) - a.items.reduce((acc, curr) => acc + curr.netPrice, 0),
    );
