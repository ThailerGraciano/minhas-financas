...
export function DatePicker({ id, name, defaultValue, value, onChange, onKeyDown, required }: DatePickerProps) {
  ...
  const date = isControlled ? (value ? parse(value, "yyyy-MM-dd", new Date()) : undefined) : internalDate;
  ...
  return (
    <>
      ...
      {name && <input type="hidden" name={name} value={date ? format(date, "yyyy-MM-dd") : ""} required={required} />}
    </>
  );
}
