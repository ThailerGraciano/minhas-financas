...
export function AccountVsGlobalExpenseChart({ initialData, competencyMonth }: AccountVsGlobalExpenseChartProps) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-card rounded-3xl sm:rounded-[2rem] px-4 py-6 sm:p-6 border-transparent shadow-sm flex items-center justify-center h-[260px] text-sm text-muted-foreground w-full min-w-0">
    ...
    <div className="bg-card rounded-3xl sm:rounded-[2rem] px-4 py-6 sm:p-6 border-transparent shadow-sm w-full min-w-0">
      ...
      <ChartContainer config={chartConfig} className="min-h-[260px] w-full" style={{ opacity: isPending ? 0.6 : 1 }}>
        <ComposedChart accessibilityLayer data={data} margin={{ top: 12, right: 16, left: 8, bottom: 0 }}>
          ...
          <Bar dataKey="income" name="income" fill="var(--color-income)" radius={[4, 4, 0, 0]} maxBarSize={60} stroke="transparent" />
          <Line 
            type="monotone" 
